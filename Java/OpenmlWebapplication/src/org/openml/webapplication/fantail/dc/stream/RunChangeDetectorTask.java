...
public class RunChangeDetectorTask extends AbstractOptionHandler {
	...
	public ClassOption driftDetector1Option = new ClassOption("NaiveBayesDdm",
			'a', "Calculates meta feature based on NaiveBayes/DDM",
			Classifier.class, "drift.DriftDetectionMethodClassifier");
	...
	public ClassOption driftDetector3Option = new ClassOption("HoeffdingDDM",
			'c', "Calculates meta feature based on Hoeffding/DDM",
			Classifier.class,
	...
	public Map<String, Double> characterize(Instances instances) {
		...
		// now fill global characteristics, just by summing all interval
		// measures
		for (Integer interval : intervalCharacteristics.keySet()) {
			...
			for (String measure : probe.keySet()) {
				if (globalCharacteristics.containsKey(measure) == false) {
					...
					globalCharacteristics.put(
						measure, 
						globalCharacteristics.get(measure) + probe.get(measure)
					);
				}
			}
		}
