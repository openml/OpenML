...
public class Main {
	...
	private final DataSplits subsamples;

	public Main(int task_id, Classifier classifier) throws Exception {
		final Config config = new Config();
		connector = new OpenmlConnector(config.getServer(), config.getApiKey());
		subsamples = new DataSplits(connector, task_id);
		...
		for (int repeat = 0; repeat < subsamples.REPEATS; ++repeat) {
			for (int fold = 0; fold < subsamples.FOLDS; ++fold) {
				...
				File trainingSetFile = Conversion.stringToTempFile(trainingSet.toString(),
						"task_1_train_" + repeat + "_" + fold, "arff");
				File testSetFile = Conversion.stringToTempFile(testSet.toString(), "task_1_test_" + repeat + "_" + fold,
						"arff");

				Instances predictionsSample = runAlgorithm(path + executable, trainingSetFile.getAbsolutePath(),
		...
		outputFiles.put("predictions",
				Conversion.stringToTempFile(predictionsAll.toString(), "predictions_executor", "arff"));
		Run description = new Run(task_id, null, FLOW_ID, executable, null, null);

		Conversion.log("OK", "UploadRun", "Will start to upload run");
		UploadRun ur = connector.openmlRunUpload(
				Conversion.stringToTempFile(xstream.toXML(description), "description_executor", "xml"), outputFiles);
		Conversion.log("OK", "Done", "Process finished, run id = " + ur.getRun_id());
	}

	public Instances runAlgorithm(String executable, String pathTrainingSet, String pathTestSet)
	...
	private void postProcess(Instances predictions, int repeat, int fold, int sample) throws Exception {
		...
		if (rowids.size() != predictions.size()) {
			throw new RuntimeException(
					"Prediction Count does not match! (" + repeat + ", " + fold + ", " + sample + ")");
		}
