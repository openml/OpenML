...
public class Setups {
	...
	private static final String TAG_NAME = "curves2";
	
	public static void main( String[] args ) throws Exception {
		//String arraystring = Arrays.toString( setups );
		//sql("SELECT s.sid FROM algorithm_setup s WHERE sid IN ( " + arraystring.substring( 1, arraystring.length() - 1 ) + " )");
	
		String sql = "SELECT s.sid,i.name,s.setup_string FROM `algorithm_setup` s, implementation i WHERE s.implementation_id = i.id AND i.dependencies = 'Weka_3.7.13'";
		sql(sql);
	}
