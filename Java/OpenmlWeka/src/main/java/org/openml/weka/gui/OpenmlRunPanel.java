...
public class OpenmlRunPanel extends JPanel implements ActionListener {
	...
	/*
	 * A class that handles running a copy of the experiment in a separate
	 * thread
	 */
				...
				if (m_ExpCopy instanceof RemoteExperiment) {
					...
					((RemoteExperiment) m_ExpCopy)
							.addRemoteExperimentListener(new RemoteExperimentListener() {
								public void remoteExperimentStatus(
										RemoteExperimentEvent e) {
					...
					while (m_RunThread != null && m_ExpCopy.hasMoreIterations()) {
						try {
							...
							if (m_ExpCopy.getUsePropertyIterator()) {
								...
								String ctype = m_ExpCopy.getPropertyArray()
								...
								String cname = " "
										...
										+ (cnum + 1)
										+ ":"
										+ m_ExpCopy.getPropertyArrayValue(cnum)
												.getClass().getName();
								current += cname;
							}
							...
							if (continueAfterError) {
								m_ExpCopy.advanceCounters(); // Try to keep
																// plowing
																// through
							} else {
								m_RunThread = null;
	 ...
	 */
	public OpenmlRunPanel(Experiment exp) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == m_StartBut) {
			if (m_RunThread == null) {
				...
				if (OpenmlExperimenter.m_Memory.memoryIsLow()) {
					proceed = OpenmlExperimenter.m_Memory.showMemoryIsLow();
				}
				if (proceed) {
					try {
						m_RunThread = new ExperimentRunner(m_Exp);
						m_RunThread.setPriority(Thread.MIN_PRIORITY); // UI has
																		// most
																		// priority
						m_RunThread.start();
					} catch (Exception ex) {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (readExp && (expFile.length() == 0)) {
				throw new Exception(
						"A filename must be given with the -f option");
			}
			Experiment exp = null;
			if (readExp) {
			...
			final OpenmlRunPanel sp = new OpenmlRunPanel(exp);
