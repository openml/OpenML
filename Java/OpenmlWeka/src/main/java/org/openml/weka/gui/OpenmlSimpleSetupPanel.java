...
public class OpenmlSimpleSetupPanel extends SimpleSetupPanel {
	...
	protected OpenmlSetupModePanel m_modePanel = null;
	...
	/**
	 * A filter to ensure only experiment (in KOML format) files get shown in
	 * the chooser
	 */
	...
	protected JFileChooser m_DestFileChooser = new JFileChooser(new File(
			System.getProperty("user.dir")));
	...
	protected JRadioButton m_ExpClassificationRBut = new JRadioButton(
			"Classification");
	...
	protected JRadioButton m_OrderDatasetsFirstRBut = new JRadioButton(
			"Data sets first");

	/** Radio button for choosing algorithms first in order of execution */
	protected JRadioButton m_OrderAlgorithmsFirstRBut = new JRadioButton(
			"Algorithms first");
	...
	public OpenmlSimpleSetupPanel() {
		...
		try {
			m_destinationDatabaseURL = (new DatabaseResultListener())
					.getDatabaseURL();
		} catch (Exception e) {
		}
		...
		m_NewBut.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				setSelectedItem(m_ResultsDestinationCBox,
						ExperimenterDefaults.getDestination());
				destinationTypeChanged();

				setSelectedItem(m_ExperimentTypeCBox,
						ExperimenterDefaults.getExperimentType());
		...
		if (ExperimenterDefaults.getExtension().equals(".xml"))
			m_FileChooser.setFileFilter(m_XMLFilter);
				...
				&& ExperimenterDefaults.getExtension().equals(
						KOML.FILE_EXTENSION))
		...
		m_ResultsDestinationPathTField.getDocument().addDocumentListener(
		...
		m_ExperimentParameterTField.getDocument().addDocumentListener(
				new DocumentListener() {
					public void insertUpdate(DocumentEvent e) {
						expParamChanged();
					}
		...
		m_NumberOfRepetitionsTField.getDocument().addDocumentListener(
				new DocumentListener() {
					public void insertUpdate(DocumentEvent e) {
	 ...
	 * 
	 * @param cb
	 *            the combobox to set the item for
	 * @param item
	 *            the item to set active
	 */
	 ...
	 *            the panel
	 */
	public void setModePanel(OpenmlSetupModePanel modePanel) {
	...
	public boolean setExperiment(Experiment exp_old) {
		...
		if (exp.getResultListener() instanceof DatabaseResultListener) {
			...
			System.out
					.println("SimpleSetup incompatibility: unrecognised result destination");
		...
		if (exp.getResultProducer() instanceof CrossValidationResultProducer) {
			...
			if (cvrp.getSplitEvaluator() instanceof ClassifierSplitEvaluator) {
				...
				System.out
						.println("SimpleSetup incompatibility: unrecognised split evaluator");
				if (userWantsToConvert()) {
					m_ExpClassificationRBut.setSelected(true);
					m_ExpRegressionRBut.setSelected(false);
				} else {
			...
			RandomSplitResultProducer rsrp = (RandomSplitResultProducer) exp
					.getResultProducer();
			...
			System.out
					.println("SimpleSetup incompatibility: unrecognised resultProducer");
			if (userWantsToConvert()) {
				m_ExperimentTypeCBox.setSelectedItem(TYPE_CROSSVALIDATION_TEXT);
				m_ExpClassificationRBut.setSelected(true);
				m_ExpRegressionRBut.setSelected(false);
		...
		if (exp.getRunLower() == 1) {
			...
			System.out
					.println("SimpleSetup incompatibility: runLower is not 1");
			if (userWantsToConvert()) {
				exp.setRunLower(1);
		...
		if (!exp.getUsePropertyIterator()
			...
			System.out
					.println("SimpleSetup incompatibility: unrecognised property iteration");
			if (userWantsToConvert()) {
	...
	private void openExperiment() {
		...
		if (m_FileChooser.getFileFilter() == m_ExpFilter) {
			if (!expFile.getName().toLowerCase()
					.endsWith(Experiment.FILE_EXTENSION))
		...
		try {
			...
			JOptionPane.showMessageDialog(this,
					"Couldn't open experiment file:\n" + expFile
							+ "\nReason:\n" + ex.getMessage(),
					"Open Experiment", JOptionPane.ERROR_MESSAGE);
			// Pop up error dialog
		}
	}
		...
		try {
			...
			JOptionPane.showMessageDialog(this,
					"Couldn't save experiment file:\n" + expFile
							+ "\nReason:\n" + ex.getMessage(),
					"Save Experiment", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void destinationAddressChanged() {
		...
		if (m_ResultsDestinationCBox.getSelectedItem() == DEST_DATABASE_TEXT) {
			...
			if (m_Exp.getResultListener() instanceof DatabaseResultListener) {
				((DatabaseResultListener) m_Exp.getResultListener())
						.setDatabaseURL(m_destinationDatabaseURL);
			}
		} else if (m_ResultsDestinationCBox.getSelectedItem() != DEST_OPENML_TEXT) {
			...
			if (m_destinationFilename.equals("")) {
				try {
					if (m_ResultsDestinationCBox.getSelectedItem() == DEST_ARFF_TEXT) {
						resultsFile = File.createTempFile("weka_experiment",
								".arff");
					}
					if (m_ResultsDestinationCBox.getSelectedItem() == DEST_CSV_TEXT) {
						resultsFile = File.createTempFile("weka_experiment",
					...
					System.err
							.println("Cannot create temp file, writing to standard out.");
			...
			((CSVResultListener) m_Exp.getResultListener())
					.setOutputFile(resultsFile);
			((CSVResultListener) m_Exp.getResultListener())
					.setOutputFileName(m_destinationFilename);
		}
	...
	private void expParamChanged() {
		...
		if (m_ExperimentTypeCBox.getSelectedItem() == TYPE_CROSSVALIDATION_TEXT) {
			try {
				m_numFolds = Integer.parseInt(m_ExperimentParameterTField
						.getText());
			...
			try {
				m_trainPercent = Double.parseDouble(m_ExperimentParameterTField
						.getText());
			} catch (NumberFormatException e) {
				return;
			}
		...
		if (m_ExperimentTypeCBox.getSelectedItem() == TYPE_CROSSVALIDATION_TEXT) {
			...
			if (m_Exp.getResultProducer() instanceof CrossValidationResultProducer) {
				CrossValidationResultProducer cvrp = (CrossValidationResultProducer) m_Exp
						.getResultProducer();
				cvrp.setNumFolds(m_numFolds);
			} else {
				return;
			...
			if (m_Exp.getResultProducer() instanceof RandomSplitResultProducer) {
				RandomSplitResultProducer rsrp = (RandomSplitResultProducer) m_Exp
						.getResultProducer();
				rsrp.setRandomizeData(m_ExperimentTypeCBox.getSelectedItem() == TYPE_RANDOMSPLIT_TEXT);
				rsrp.setTrainPercent(m_trainPercent);
			} else {
	...
	private void numRepetitionsChanged() {
		...
		try {
			m_numRepetitions = Integer.parseInt(m_NumberOfRepetitionsTField
					.getText());
		} catch (NumberFormatException e) {
			return;
		}
	...
	private void chooseDestinationFile() {
		...
		m_ResultsDestinationPathTField.setText(m_DestFileChooser
