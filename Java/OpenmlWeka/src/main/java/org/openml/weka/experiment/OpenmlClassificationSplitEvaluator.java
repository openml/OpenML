...
public class OpenmlClassificationSplitEvaluator extends ClassifierSplitEvaluator implements OpenmlSplitEvaluator {
	...
	/**
	 * JvR: Direct copies from weka.experiment.ClassifierSplitEvaluator. Change
	 * values if these also change there.
	 */
	...
	public Object[] getResult(Instances train, Instances test) throws Exception {
		...
		/**
		 * ADDED BY JvR: In between the upcoming comment blocks, the only
		 * additions to this class are done.
		 **/
		// training classifier
		...
		result[current++] = new Double(
				eval.truePositiveRate(getClassForIRStatistics()));
		result[current++] = new Double(
				eval.numTruePositives(getClassForIRStatistics()));
		result[current++] = new Double(
				eval.falsePositiveRate(getClassForIRStatistics()));
		result[current++] = new Double(
				eval.numFalsePositives(getClassForIRStatistics()));
		result[current++] = new Double(
				eval.trueNegativeRate(getClassForIRStatistics()));
		result[current++] = new Double(
				eval.numTrueNegatives(getClassForIRStatistics()));
		result[current++] = new Double(
				eval.falseNegativeRate(getClassForIRStatistics()));
		result[current++] = new Double(
				eval.numFalseNegatives(getClassForIRStatistics()));
		result[current++] = new Double(
				eval.precision(getClassForIRStatistics()));
		...
		result[current++] = new Double(
				eval.areaUnderROC(getClassForIRStatistics()));
		result[current++] = new Double(
				eval.areaUnderPRC(getClassForIRStatistics()));
		...
		if (getAttributeID() >= 0) {
			...
			if (test.attribute(getAttributeID()).isNumeric()) {
				...
				for (int i = 1; i < test.numInstances(); i++) {
					idsString += "|"
							+ test.instance(i).stringValue(getAttributeID());
				}
			}
