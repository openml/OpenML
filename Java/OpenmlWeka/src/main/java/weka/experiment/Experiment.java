...
public class Experiment implements Serializable, OptionHandler, RevisionHandler {
	 ...
	 * involved in the experiment. This information is not output with the
	...
	/**
	 * Gets the path of properties taken to get to the custom property to
	 * iterate over.
	 * 
	...
	/**
	 * Sets the path of properties taken to get to the custom property to
	 * iterate over.
	 * 
	 ...
	 * 
	 * @param newPropArray
	 *            a value of type Object which should be an array of the
	 *            appropriate values.
	 */
	 ...
	 * 
	 * @return a value of type Object which should be an array of the
	 *         appropriate values.
	 */
	...
	/**
	 * When an experiment is running, this returns the index of the current
	 ...
	 * 
	 * @throws Exception
	 *             if additional measures don't comply to the naming convention
	 *             (starting with "measure")
	 */
	...
	/**
	 * Recursively sets the custom property value, by setting all values along
	 * the property path.
	 * 
	 * @param propertyDepth
	...
	private void nextIterationDatasetBased() throws Exception {
			...
			try {
				if (verbose) {
					...
					if (getUsePropertyIterator()) {
						...
						String ctype = getPropertyArray().getClass()
						...
						String cname = " "
								...
								+ (cnum + 1)
								+ ":"
								+ getPropertyArrayValue(cnum).getClass()
										.getName();
						current += cname;
					}
	...
	public Enumeration listOptions() {
		...
		newVector.addElement(new Option(
				"\tThe dataset to run the experiment on.\n"
						+ "\t(required, may be specified multiple times)", "T",
				1, "-T <arff file>"));
		newVector.addElement(new Option(
				"\tThe full class name of a ResultProducer (required).\n"
						+ "\teg: weka.experiment.RandomSplitResultProducer",
	...
	public void setOptions(String[] options) throws Exception {
		...
		// RP.
		// GHF -- nice idea, but it prevents you from using result producers
		// that
	...
	public String[] getOptions() {
		...
		String[] options = new String[rpOptions.length + getDatasets().size()
				* 2 + 11];
		int current = 0;
	 ...
	 * 
	 * @param filename
	 *            the file to load the experiment from
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (expFile.length() == 0) {
				...
				// allow extra datasets to be added to pre-loaded experiment
				// from command line
				String dataName;
				do {
